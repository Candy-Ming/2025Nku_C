...
void mazeWidget::paintEvent(QPaintEvent*) {
    ...
    for(int i = 0; i < map->getlevel() * 2 + 1; i++) {
        for(int j = 0; j < map->getlevel() * 2 + 1; j++) {
            ...
            case 7: brush = Qt::yellow; break;
            case 3: case 4: brush = Qt::white; break;
            case 5: brush = Qt::blue; break;
            case 6: brush = Qt::green; break;
            default: brush = Qt::gray; break;
            }
            if (i == map->p_x && j == map->p_y) brush = Qt::red;
            painter.fillRect(start_x + i * perblock, start_y + j * perblock, perblock, perblock, brush);
        }
    }
}
